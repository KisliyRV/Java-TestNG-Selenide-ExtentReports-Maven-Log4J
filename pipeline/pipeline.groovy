...
//            echo "Copied file: '${copiedFile}'"
            ...
            if (copiedFile) {
                ...
                def xml = parser.parseText(copiedFile)
                def testElement = xml.test
                scope = testElement.classes.class*.@name.collect { it.text().replace("ua.com.usource.tests.", "") }.join(",").replaceFirst(",", "")
            } else {
                scope = null
            }
